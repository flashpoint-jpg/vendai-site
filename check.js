...
async function calcularFreteCompra(){const p=produtoLojaAtual;if(!p)return;const qtd=Number(document.getElementById('compraQtdLoja').value)||1,end=document.getElementById('compraEndereco').value.trim(),box=document.getElementById('compraCotacao');if(end.length<5){showToast('Informe o endereço completo');return;}box.textContent='Calculando a rota e o frete do Entrega Flash...';try{const q=await vendaiEntrega('quote',{produto_id:p.id,quantidade:qtd,endereco_entrega:end});ultimaCotacaoFlash=q;box.innerHTML=`<strong>Entrega Flash: ${formatPrice(q.frete_valor)}</strong><br>Distância pela rota: <strong>${Number(q.distancia_km||0).toFixed(1)} km</strong> · ${nomeVeiculo(q.veiculo)} · ${q.motoristas_disponiveis} entregador(es) disponível(is) na região<br><strong>Total estimado: ${formatPrice(Number(p.preco)*qtd+Number(q.frete_valor))}</strong>`;}catch(e){ultimaCotacaoFlash=null;const m=String(e.message||'');box.textContent=m.includes('sem_motorista')?'Não há motorista Entrega Flash disponível para esse veículo nessa região agora.':m.includes('distancia_fora_da_operacao')?'O endereço está fora da área atendida pelo Entrega Flash.':m.includes('tabela_frete_indisponivel')?'A tabela de frete desse veículo ainda não está configurada.':m.includes('produto_indisponivel')?'Este produto não está mais disponível nessa quantidade.':m.includes('entrega_flash_indisponivel')?'Essa loja não está com Entrega Flash ativo no momento.':m.includes('sessao_invalida')?'Atualize a página e tente calcular novamente.':m.includes('endereco')?'Não consegui localizar esse endereço. Informe rua, número, bairro, cidade e CEP.':'Não consegui calcular o frete agora. Confira o endereço e tente novamente.';}}
